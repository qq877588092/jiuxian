class MainBox {
    ...
    createMain() {
        var html = this.data.map((ele, index) => {
            ...
            var html10 = ele.bottomRight.bottomImg.map((item, index) => {
                return `
                  <div class="topTenPic"><a href="#" target="_blank"
                      title="【买一送一】53°国台·品鉴15  500ml"><img
                        src=${item} width="90"
                        height="90" alt="【买一送一】53°国台·品鉴15  500ml"></a></div>
                `
            }).join(",");

            var item10 = ele.bottomRight.bottomImg.map((item, index) => {
                for (var i = 0; i <= index; i++) {
                    return `
